    ...
    <div v-if="dataLoaded">
      ...
      >
        ...
        <div v-if="user" class="flex absolute left-2 top-2 gap-x-2">
          <!-- This edit ui will only render if the click event is detected -->
          <div
        ...
        <!-- New inputs if edit mode has been toggled -->
  ...
  setup() {
    ...
    const getData = async () => {
      try {
        ...
        data.value = workouts[0]; // We only want the first item in the array to render when viewing the workout
    ...
    const addExercise = () => {
      ...
      // The only other option is cardio, so no need for an else-if
      data.value.exercises.push({
        id: uid(),
        cardioType: '',
        distance: '',
    ...
    // Delete exercise
      const deleteExercise = (id) => {
        if (data.value.exercises.length > 1) { // Must have at least two exerces inside of their workout
          data.value.exercises = data.value.exercises.filter((exercise) => exercise.id !== id) // This will return an array of all of the exercises that do not match the id passed in
          return;
        }
